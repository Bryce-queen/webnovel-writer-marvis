...
| 命名 | 开始命名 | `python -X utf8 "{SKILL_ROOT}/scripts/reference_search.py" --skill init --table 命名规则 --query "{命名对象} {题材}" --genre {题材}` |
...
if [ -z "{SKILL_ROOT}" ] || [ ! -d "{SKILL_ROOT}/scripts" ]; then
  echo "ERROR: 未设置 SKILL_ROOT 或缺少目录: {SKILL_ROOT}/scripts" >&2
  exit 1
fi
...
必须做：
- 确认当前目录可写；确认入口脚本 `{SKILL_ROOT}/scripts/webnovel.py` 存在（仅支持插件目录）。
...
- 禁止在插件目录（`{SKILL_ROOT}`）下生成项目文件；禁止直接把 `WORKSPACE_ROOT` 当作 `PROJECT_ROOT`，除非用户明确指定当前目录就是书项目根。
...
参数全部来自上面的采集对象（书名/题材/主角/金手指/世界观/反派/创意约束等），逐字段映射为 `webnovel.py init` 的 `--*` 选项；完整字段清单见 `references/init-collection-schema.md`，可用 `python "{SKILL_ROOT}/scripts/webnovel.py" init --help` 核对选项名。

```bash
python "{SKILL_ROOT}/scripts/webnovel.py" init \
