...
def main() -> int:
    ...
    plugin_root = Path(
        ...
        or (Path(__file__).resolve().parents[2] if (Path(__file__).resolve().parents[2] / "scripts" / "webnovel.py").is_file() else None)
        or Path(__file__).resolve().parents[1]
    )
    workspace_root = os.environ.get("MARVIS_PROJECT_DIR") or os.environ.get("CLAUDE_PROJECT_DIR") or os.getcwd()
