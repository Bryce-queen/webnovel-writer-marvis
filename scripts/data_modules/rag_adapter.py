...
class RAGAdapter:
    ...
    def _update_degraded_mode(self) -> None:
        ...
        rerank_client = getattr(self.api_client, "_rerank_client", None)
        rerank_status = getattr(rerank_client, "last_error_status", None)
        if rerank_status == 401:
            self._degraded_mode_reason = "rerank_auth_failed"
        # 主动检测：Rerank API Key 未配置（不等 401）
        if self._degraded_mode_reason is None:
            rerank_key = str(getattr(self.config, "rerank_api_key", "") or "").strip()
            rerank_url = str(getattr(self.config, "rerank_base_url", "") or "").strip().lower()
            if not rerank_key and ("jina" in rerank_url or "cohere" in rerank_url):
                self._degraded_mode_reason = "rerank_auth_failed"
