...
def _get_user_claude_root() -> Path:
    raw = os.environ.get("WEBNOVEL_MARVIS_HOME") or os.environ.get("MARVIS_HOME") or os.environ.get("WEBNOVEL_CLAUDE_HOME") or os.environ.get("CLAUDE_HOME")
    if raw:
        try:
...
def _load_dotenv():
    ...
    约定：
    - 项目级 `.env`（当前工作目录下）优先；
    - 全局 `.env` 作为兜底：`~/.marvis/webnovel-writer/.env`
    """
...
def get_config(project_root: Optional[Path] = None) -> DataModulesConfig:
    ...
    if _default_config is None:
        ...
        # - 支持 `.webnovel-current-project` / `.claude/.webnovel-current-project` 指针文件
