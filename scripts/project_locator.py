...
CURRENT_PROJECT_POINTER_MARVIS: Path = Path(".webnovel-current-project")  # Marvis 工作区 pointer（无 .claude/ 中间层）

# 用户级全局映射（当 skills/agents 安装在 ~/.marvis 或 ~/.claude 时，项目目录可能在任意盘符）
# 该文件用于在“空上下文 + CWD 不在项目内”的情况下仍能定位到正确 project_root。
GLOBAL_REGISTRY_REL: Path = Path("webnovel-writer") / "workspaces.json"

# 工作区根目录环境变量（优先 MARVIS，兼容旧 CLAUDE 变量名）
ENV_MARVIS_PROJECT_DIR = "MARVIS_PROJECT_DIR"
ENV_CLAUDE_PROJECT_DIR = "CLAUDE_PROJECT_DIR"
ENV_MARVIS_HOME = "MARVIS_HOME"
ENV_CLAUDE_HOME = "CLAUDE_HOME"
ENV_WEBNOVEL_MARVIS_HOME = "WEBNOVEL_MARVIS_HOME"
...
def _get_user_claude_root() -> Path:
    raw = os.environ.get(ENV_WEBNOVEL_MARVIS_HOME) or os.environ.get(ENV_MARVIS_HOME) or os.environ.get(ENV_WEBNOVEL_CLAUDE_HOME) or os.environ.get(ENV_CLAUDE_HOME)
    if raw:
        try:
            return normalize_windows_path(raw).expanduser().resolve()
        except Exception:
            return normalize_windows_path(raw).expanduser()
    return (Path.home() / ".marvis").resolve()
...
def _resolve_project_root_from_global_registry(
    ...
    *,
    ...
    安全策略：
    - 优先使用 workspace_hint / MARVIS_PROJECT_DIR 提示做匹配。
    ...
    hints: list[Path] = []
    env_ws = os.environ.get(ENV_MARVIS_PROJECT_DIR) or os.environ.get(ENV_CLAUDE_PROJECT_DIR)
    if env_ws:
...
def _pointer_candidates(cwd: Path, *, stop_at: Optional[Path] = None) -> Iterable[Path]:
    ...
    for candidate in (cwd, *cwd.parents):
        yield candidate / CURRENT_PROJECT_POINTER_MARVIS  # Marvis pointer（优先）
        yield candidate / CURRENT_PROJECT_POINTER_REL     # Claude Code pointer
...
def _resolve_project_root_from_pointer(cwd: Path, *, stop_at: Optional[Path] = None) -> Optional[Path]:
    ...
    - relative path is also supported (resolved relative to pointer's parent dir).
    """
...
def _find_workspace_root_with_claude(start: Path) -> Optional[Path]:
    """Find nearest ancestor containing `.claude/` or Marvis workspace."""
    # Marvis 环境：MARVIS_PROJECT_DIR 即工作区根目录
    env_ws = os.environ.get(ENV_MARVIS_PROJECT_DIR)
    if env_ws:
        p = normalize_windows_path(env_ws).expanduser()
        try:
            p = p.resolve()
        except Exception:
            pass
        if p.is_dir():
            return p
    # 传统 Claude Code 环境：向上查找 .claude/ 目录
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    if ws_root is None:
        # 兜底：若无法找到工作区根目录，将项目父目录视为“工作区”候选，
        # 仅用于写入用户级 registry（不写 pointer 文件）。
        ws_root = root.parent if root.parent != root else None
    # 注意：ws_root 可能为 None（例如全局安装的 skills/agents）。
    # 这类情况仍然需要写入用户级 registry，以支持后续“空上下文”定位。

    pointer_file: Optional[Path] = None
    if ws_root is not None:
        # 判断是 Marvis 还是 Claude Code 工作区，写 pointer 到对应路径
        if os.environ.get(ENV_MARVIS_PROJECT_DIR):
            # Marvis 环境：pointer 直接写在工作区根目录（无 .claude/ 中间层）
            try:
                pointer_file = ws_root / CURRENT_PROJECT_POINTER_MARVIS
                pointer_file.write_text(str(root), encoding="utf-8")
            except Exception:
                pointer_file = None
        elif (ws_root / ".claude").is_dir():
            # Claude Code 环境：pointer 写在 .claude/ 子目录
            try:
...
def resolve_project_root(explicit_project_root: Optional[str] = None, *, cwd: Optional[Path] = None) -> Path:
    ...
    # - 若 MARVIS_PROJECT_DIR 存在：认为提供了工作区上下文
    # - 否则仅在 base 位于某个已记录 workspace 内时启用（前缀匹配）
    allow_last_used = bool(os.environ.get(ENV_MARVIS_PROJECT_DIR) or os.environ.get(ENV_CLAUDE_PROJECT_DIR))
    reg_root = _resolve_project_root_from_global_registry(
        base,
        workspace_hint=None,
